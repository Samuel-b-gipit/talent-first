...
export async function GET() {
  try {
    ...
    const talents = await prisma.talentProfile.findMany({
      select: { skills: true, rate: true },
    });
    const skillStats: Record<
      string,
      { count: number; totalRate: number; talentCount: number }
    > = {};
    for (const t of talents) {
      for (const skill of t.skills) {
        if (!skillStats[skill])
          skillStats[skill] = { count: 0, totalRate: 0, talentCount: 0 };
    ...
    return NextResponse.json(
      { error: "Failed to fetch market insights", details: error },
      { status: 500 },
    );
  }
}
