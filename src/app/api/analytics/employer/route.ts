...
export async function GET(req: NextRequest) {
  ...
  if (user.role !== "EMPLOYER") {
    return NextResponse.json(
      { error: "Forbidden: Only employers can view analytics" },
      { status: 403 },
    );
  }
  ...
  try {
    ...
    const responseTimes = proposals
      .filter(
        (p: Proposal) => p.status !== "PENDING" && p.createdAt && p.updatedAt,
      )
      .map(
        (p: Proposal) =>
    ...
    return NextResponse.json(
      { error: "Failed to fetch employer analytics", details: error },
      { status: 500 },
    );
  }
}
